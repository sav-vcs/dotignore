...
impl IgnoreParser {
    ...
    pub fn parse_file(&self, path: &Path) -> Result<DotIgnore> {
        ...
        for pattern in ignore_file.patterns() {
            dotignore.add_pattern(pattern.clone());
        }
        
        for group in ignore_file.groups() {
            dotignore.add_group(group.clone());
        }
...
"#;
        ...
        let dotignore = parser.parse_file(file.path()).unwrap();
        
        // Check patterns and groups
        assert_eq!(dotignore.patterns.len(), 0);
        assert_eq!(dotignore.groups.len(), 2);
        
        // Ensure groups have the correct patterns
        let standard_group = dotignore.groups.iter().find(|g| g.name == "standard_patterns").unwrap();
        assert_eq!(standard_group.patterns.len(), 3);

        let windows_group = dotignore.groups.iter().find(|g| g.name == "platform:windows").unwrap();
        assert_eq!(windows_group.patterns.len(), 2);
        
        // Check ignore functionality
        assert!(dotignore.is_ignored("file.txt"));
        assert!(!dotignore.is_ignored("important.txt"));
        assert!(dotignore.is_ignored("temp/file"));
        assert!(dotignore.is_ignored("Thumbs.db"));
        assert!(!dotignore.is_ignored("file.rs"));
    }
...
"#;
        ...
        let parser = IgnoreParser::new();
        let dotignore = parser.parse_string(content).unwrap();
        
        // Check we have expected patterns and groups
        assert_eq!(dotignore.patterns.len(), 0);
        assert_eq!(dotignore.groups.len(), 1);
...
"#;
        ...
        assert!(dotignore.groups.iter().any(|g| g.name == "standard_patterns"));
        assert!(dotignore.groups.iter().any(|g| g.name == "unclosed_group"));
        
        // Verify the unclosed group has patterns
        let unclosed = dotignore.groups.iter().find(|g| g.name == "unclosed_group").unwrap();
