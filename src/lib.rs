...
pub mod plugin {
    ...
    impl IgnoreRules {
        ...
        pub fn from_DotIgnore(dotignore: &DotIgnore) -> Self {
            let mut rules = Self::new();
            
            // Add patterns
            for pattern in &dotignore.patterns {
                rules.patterns.push(pattern.clone());
            }
            
            // Add patterns from groups
            for group in &dotignore.groups {
    ...
    impl IgnorePlugin for DotIgnorePlugin {
        fn parse_ignore_file(&self, path: &Path) -> Result<IgnoreRules, IgnoreError> {
            ...
            let dotignore = DotIgnore::load_from_file(path)
                .map_err(|e| IgnoreError::Other(e))?;
                
            Ok(IgnoreRules::from_DotIgnore(&dotignore))
        }
...
"#;
        ...
        let dotignore = DotIgnore::load_from_file(&ignore_path).unwrap();
