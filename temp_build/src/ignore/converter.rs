...
impl IgnoreConverter {
    ...
    fn convert_from_git(&self, content: &str) -> (String, PatternStatistics) {
        ...
        while let Some(line) = lines.next() {
            ...
            // Skip empty lines
            if trimmed.is_empty() {
                current_group = None; // Reset current group on empty lines
                continue;
            }
            ...
            if trimmed.starts_with('#') {
                ...
                if !comment.is_empty() {
                    // Convert comment to group name (lowercase, replace spaces with underscores)
                    let group_name = comment.to_lowercase()
                        .replace(' ', "_")
            ...
            // Add pattern to current group or default
            if let Some(group) = &current_group {
                if let Some(group_patterns) = groups.get_mut(group) {
                    group_patterns.push(pattern);
                }
            } else {
                default_group.push(pattern);
        ...
        for (group_name, patterns) in groups {
            if !patterns.is_empty() {
                converted.push_str(&format!("[{}] {{\n", group_name));
                converted.push_str("    # Patterns from .gitignore\n");
